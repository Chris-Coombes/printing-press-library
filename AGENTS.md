...
If you modify a published CLI under `library/<cat>/<slug>/` beyond what the generator produced, record each customization in a `.printing-press-patches.json` at the CLI's root (parallel to `.printing-press.json`) so it isn't lost on the next regen and is visible to the next reader. SKILL.md / README.md edits are owned by `tools/sweep-canonical/` or direct edit and don't need a patch manifest; this convention is for code-level customizations. Minimum shape:
...
This file is the **index of customizations**, not a second copy of the diff. Diffs live in `git`; code lives in the source files; the manifest entry's `summary` + `reason` + `validated_outcome` carries the local semantics. Keep them short — if you find yourself writing tables of field renames or SQL transformations, that detail belongs in the commit message, not here. A fresh print from the generator overwrites this tree, and the manifest is what tells the next agent (or regeneration tooling) what was customized and why.

There used to be a parallel `// PATCH:` source-comment convention enforced by `verify_publish_package.py`. It was removed — the manifest entry is sufficient as the audit record, and the source-comment requirement was duplicative and inconsistently applied across the published library. Existing `// PATCH` comments in already-published CLIs are fine to leave in place; new patches don't need to add them.
